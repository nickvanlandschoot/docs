...
curl -H "X-API-Key: your-api-key" \
  "https://api.corpus.intrace.ai/v1/corpus/events?event_category=conflict&event_category=crime&country=UA&country=RU"
```
...
The `/v1/corpus/actors` endpoint supports:
...
The `/v1/corpus/actors/{id}/events` endpoint accepts the same filter and sort parameters as `/v1/corpus/events`.
...
types = requests.get(
    f"{BASE_URL}/v1/corpus/metadata/event-types",
    headers=HEADERS,
...
for event in events["items"]:
    event_detail = requests.get(
        f"{BASE_URL}/v1/corpus/events/{event['id']}",
        headers=HEADERS
    ).json()
...
if top_actor:
    ...
    actor_events = requests.get(
        f"{BASE_URL}/v1/corpus/actors/{top_actor['id']}/events",
        headers=HEADERS,
        params={
            "event_category": "conflict",
            "date_from": "2025-01-01",
    ...
    csv_response = requests.get(
        f"{BASE_URL}/v1/corpus/events/export",
        headers=HEADERS,
        params={
            "format": "csv",
            "event_category": "conflict",
...
class IntraceCorpusClient {
  ...
  async listEvents(params: {
    ...
    const response = await fetch(`${this.baseUrl}/v1/corpus/events?${q}`, {
  ...
  async getEvent(eventId: string): Promise<any> {
    const response = await fetch(`${this.baseUrl}/v1/corpus/events/${eventId}`, {
      headers: this.headers,
    });
    if (!response.ok) throw new Error(`API error ${response.status}: ${response.statusText}`);
    return response.json();
  ...
  async exportEvents(params: {
    ...
    const response = await fetch(`${this.baseUrl}/v1/corpus/events/export?${q}`, {
      headers: this.headers,
    });
    if (!response.ok) throw new Error(`API error ${response.status}: ${response.statusText}`);
