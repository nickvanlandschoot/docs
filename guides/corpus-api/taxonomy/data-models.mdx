...
**Note:** The actor type/category is not included in the response object. To filter actors by type, use the `actor_type` query parameter when calling `/v1/corpus/actors`, and use the `/v1/corpus/metadata/actor-types` endpoint to discover available types.
...
| `principalActorCategory` | string | Category of principal actor (state_forces, rebel_group, etc.) |
| `subPrincipalActors` | string | Internal sub-units of principal actor, semicolon-separated |
...
| `subSecondaryActors` | string | Internal sub-units of secondary actor, semicolon-separated |
| `supportingSupportingActor` | string | External actors supporting secondary, comma-separated |
